...
static int ndt_burn_fw_m65(unsigned char *buf, unsigned int len, int retry);
static int ndt_read_eeprom_m65(unsigned char reg, unsigned char *datbuf, int byteno);
static int ndt_write_eeprom_m65(unsigned char reg, unsigned char *datbuf, int byteno);
static void ndt_reset_m65(void);
...
int ndt_get_pressure_m65(int touch_flag, int x, int y)
{
	int pressure;
...
static int ndt_burn_fw_m65(unsigned char *buf, unsigned int len, int retry)
{
	...
	buffer[1] = 0xa5;
	buffer[2] = 0x5a;
	ndt_write_register(0x21, buffer, 3);
	msleep(50);
	/*write eeprom*/
		pos = 0;
		reg = 0x00;
		...
		while (i2c_ok_flag && pos < len) {
			if (ndt_write_eeprom_m65(reg, buf + pos, byteno) >= 0) {
	...
	/*read eeprom and check*/
		...
		while (i2c_ok_flag && pos < len) {
			if (ndt_read_eeprom_m65(reg1, read_buf + pos, byteno) > 0) {
...
static ssize_t pressure_update_fw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret = request_firmware(&fw_entry, "ndt_fw_m65.bin", &(g_ndt_client->dev));
	if (ret != 0) {
...
{
	ndt_update_fw(false, "ndt_fw_m65.bin", 3);
}
...
static ssize_t pressure_pressure_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	pressure = ndt_get_pressure_m65(touch_down, x, y);
			    ...
			    const struct i2c_device_id *id)
{
	...
	*/
	...
	if (error < 0) {
		...
		error = ndt_update_fw(1, "ndt_fw_m65.bin", 1);
		if (error == -EINVAL) {
...
static int __init ndt_m65_force_init(void)
{
	return i2c_add_driver(&ndt_force_driver);
}

static void __exit ndt_m65_force_exit(void)
{
	i2c_del_driver(&ndt_force_driver);
}

module_init(ndt_m65_force_init);
module_exit(ndt_m65_force_exit);
